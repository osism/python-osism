...
def test_prepare_real_deep_merge_combines_base_and_config_context(prep):
    # Integration-style check with the real deep_merge: the overlay must be
    # merged into the base accumulator that is returned as ``attrs``.
    from osism.tasks.conductor.utils import deep_merge

    prep.deep_merge.side_effect = deep_merge
    base = {"driver": "redfish", "driver_info": {"redfish_address": "5.6.7.8"}}
    device = _device(config_context={"ironic_parameters": {"some_new_key": "v"}})

    attrs, _ = _prepare_node_attributes(device, lambda: base)

    assert attrs["driver"] == "redfish"
    assert attrs["driver_info"] == {"redfish_address": "5.6.7.8"}
    assert attrs["some_new_key"] == "v"
...
def test_prepare_template_vars_null_secrets_field_falls_back_to_defaults(prep):
    # "secrets" present but explicitly null in NetBox, distinct from absent.
    device = _device(custom_fields={"secrets": None})
